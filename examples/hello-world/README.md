...
Rust-backed module, two native-only modules, two standalone runtime helper crates outside
...
`atom_app(...).config_plugins`, and app-owned `atom_runtime_config()` without making framework
automation depend on app-specific generated hooks.

`apps/hello_atom` consumes the Rust-backed module metadata and the demo surface while depending only
on `atom-runtime` as a Rust crate:
...
handwritten module schema file or hand-authored FlatBuffers codecs. `#[atom_import]` parsing and
`{FnName}Args` generation are covered in `atom-cng` tests until the higher-level provider surface
lands in ALE-94/ALE-95.
