...
Atom is a ruleset and framework, not a Cargo workspace with an optional Bazel wrapper. The build
graph, metadata graph, verification flow, and example consumer all need one source of truth.
...
- Repo-local `Cargo.toml`, `Cargo.lock`, `Atom.toml`, and `Atom.module.toml` are not part of the
  contract.
