...
- First-party runtime plugins for higher-level app behavior such as navigation.
...
a concrete navigation system or renderer.
...
The framework should ship first-party plugins for common needs. The first one should be navigation,
so Phase 4 proves the extension model before a renderer exists.
...
widget commands through the same bridge. That should be treated as Phase 6, not bundled into the
first bootstrap.
...
### Phase 4B: First-party runtime plugins

Deliverables:

- Runtime plugin registration and ordering model proven in a real app
- First-party `atom-navigation` plugin
- Route stack model with push, pop, replace, reset, and back handling
- Example app integration through runtime plugins instead of ad hoc host logic

Exit criteria:

- The example app uses the first-party navigation plugin on both platforms
