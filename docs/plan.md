...
- Optional runtime support libraries outside `atom-runtime` for higher-level app behavior when apps
  need them.
...
5. Assemble runtime config in app-owned Rust through `atom_runtime_config()` and call app-owned or
   third-party runtime support libraries where needed.
...
- Route definitions or screen descriptors when using navigation-style support libraries.
- The support libraries it opts into.
- The app-owned `atom_runtime_config()` registration function or equivalent builder call that wires
  app state and modules into the runtime without the kernel knowing about concrete support
  libraries.
...
- Public free-function runtime APIs used by app-owned support libraries.
...
### 3. Runtime support libraries

A runtime support library is plain Rust code that composes with the runtime kernel through the
public `atom_runtime::*` API to provide higher-level behavior such as:
...
A runtime support library should be able to:

- Own library-local state.
- React to lifecycle or app events when the app forwards them.
- Emit state updates or events through the runtime free-function API.
- Start async work once the runtime is ready.

Runtime support libraries should live outside `atom-runtime` as separate crates or example-owned
libraries. The kernel should export the public runtime API, not concrete support libraries.

The framework should treat support libraries as normal Rust code using the same public runtime API
regardless of whether they are app-owned, example-owned, or third-party.
...
Modules are not the same thing as runtime support libraries. Modules bridge host capabilities into
Rust. Runtime support libraries compose Rust-side app behavior on top of the public runtime API.
...
- `atom-runtime` owns lifecycle, public free-function runtime APIs, and runtime/module coordination.
- `examples/hello-world/plugins/lifecycle_logger` shows support-library code can stay outside the
  kernel while using the same public runtime APIs available to app-owned or third-party crates.
...
headless app behavior. Those belong in runtime support libraries.
...
We need two distinct extension systems in addition to native modules:

1. Runtime support libraries, which extend in-process Rust app behavior through the public runtime
   API.
2. Config/CNG plugins, which shape generated native hosts.

Runtime support libraries should cover capabilities such as:
...
- Runtime support libraries should be composed from app-owned Rust code rather than through
  kernel-side discovery.
- Generated Rust app bridges should call the app entry crate identified by `entry_crate_name` rather
  than hard-coding support-library identities into the runtime kernel or native templates.
- Runtime support libraries should not mutate generated native trees directly.
...
- Native modules bridge platform capabilities and are not a substitute for runtime support
  libraries.
...
- The runtime exposes one stable execution model for apps and support libraries without requiring a
  renderer

### Phase 4B: Runtime support library integration

Deliverables:

- Stable public runtime free-function API for support libraries
- App-owned runtime config through `atom_runtime_config()` or equivalent builder
- Generated Rust app bridges call the app entry crate identified by `entry_crate_name`
- Example support-library crate outside `atom-runtime` proving the kernel does not know concrete
  library identities
- Support-library lifecycle and state integration proven without generated host changes
- Bazel guidance for publishing and consuming support-library crates as normal dependencies

Exit criteria:

- A support library can be added or removed without changing `atom-runtime` or generated
  Swift/Kotlin templates beyond the generic bridge
- The same public runtime API works for app-owned, example-owned, and third-party-style support
  libraries

### Phase 4C: Runtime support library patterns

Deliverables:

- Guidance for routing-style or workflow-style support libraries outside `atom-runtime`
- At least one additional support-library crate that is not destination-shaped, proving the model is
  general
- Example-owned or consumer-owned integration through plain Rust code instead of ad hoc host logic
- No support-library types re-exported from `atom-runtime`

Exit criteria:

- Support-library behavior remains a library concern instead of moving into the kernel
- Support libraries are consumed through the same public runtime API shape expected for app-owned or
  third-party crates
- Navigation or routing remains a library concern, not a kernel concern
...
This mirrors the runtime support library model: the kernel does not know concrete support-library
identities, and config plugins are consumed as normal Rust crate dependencies.
...
- Runtime support libraries and config plugins remain separate concepts with separate APIs
...
- The same app can combine runtime support libraries, native modules, and config/CNG plugins
  coherently
...
- `atom --version`
- `atom test`
- Runtime support library and config-plugin authoring docs
- Example workspace proving app-owned and third-party-style support-library composition
- `atom destinations --platform <platform> --json`
- `atom devices --platform <platform> --json`
...
- A new app can add a runtime support library crate and a config/CNG plugin crate without touching
  framework internals
