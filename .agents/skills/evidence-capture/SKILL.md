...
3. Run `mise exec -- scripts/run.sh screenshot <platform>`, `logs <platform>`, `video <platform>`,
   or `inspect-ui <platform>` with the chosen destination. These commands should reuse the current
   foreground app state when the selected target is already running. Prefer the focused log output
   these commands produce over raw device-wide syslog or logcat when you are validating app
   behavior. On iOS simulators, screenshot capture may fall back to `simctl io screenshot` when
   `idb` cannot encode an image. On iOS, prefer `.mov` output paths for video capture; on Android,
   prefer `.mp4`.
