...
2. If the app is not already running and you want to preserve state across multiple captures, prefer
   `atom run --platform <platform> --detach` instead of keeping a log-streaming session open.
...
4. Use `atom stop --platform <platform>` for cleanup only when the workflow explicitly launched a
   disposable detached session.
