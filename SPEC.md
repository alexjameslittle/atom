...
- The app entry crate MUST build `RuntimeConfig` in app-owned Rust code.
- The runtime singleton MUST start without kernel-side plugin registration, module registration, or
  runtime handle allocation.
- Native module metadata still flows through Section 6 and CNG, but runtime startup itself is
  singleton initialization only.
...
### 7.6 Runtime Library Model

- The runtime MUST expose public free functions for shared state writes, state reads, event
  dispatch, async work access, and `Running`-state checks.
- Runtime support crates MAY own library-local state and call those free functions to mirror app
  behavior into the runtime snapshot.
- Runtime support crates MUST use the kernel's lifecycle and dispatch semantics. They MUST NOT
  introduce a second lifecycle model or direct generated-host customization path.
- The runtime kernel MUST remain the single authority for lifecycle transitions and dispatch
  ordering even when higher-level libraries layer on top of it.
