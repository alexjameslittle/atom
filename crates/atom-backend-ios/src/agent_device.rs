...
pub(crate) fn open_app(
    ...
    let mut subcommand = vec!["open".to_owned(), bundle_id.to_owned()];
    if relaunch {
        subcommand.push("--relaunch".to_owned());
    }
    run_agent_device(runner, repo_root, destination_id, session_name, &subcommand)
}
...
pub(crate) fn close_app(
    ...
    run_agent_device(
        ...
        destination_id,
        session_name,
        &["close".to_owned(), bundle_id.to_owned()],
    )
}
...
fn run_agent_device(
    ...
    let args = agent_device_args(repo_root, destination_id, session_name, subcommand)?;
...
mod tests {
    ...
    use super::{
        ...
        interact_with_agent_device, list_destinations, logs_path,
    };
  ...
  "data": {
    "path": "/tmp/agent-device.log"
        ...
        let path = logs_path(root, "SIM-1", "atom-test", &mut runner).expect("log path");

        assert_eq!(path.as_str(), "/tmp/agent-device.log");
    }
}
