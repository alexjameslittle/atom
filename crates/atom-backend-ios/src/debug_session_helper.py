...
class DebugSession:
    ...
    def pause(self) -> dict:
        ...
        if error.Fail():
            raise HelperError(
                ...
                error.GetCString() or "failed to interrupt the iOS process",
            )
    ...
    def resume(self) -> dict:
        ...
        if error.Fail():
            raise HelperError(
                "external_tool_failed",
                error.GetCString() or "failed to continue the iOS process",
            )
    ...
    def resolve_thread(self, thread_id: Optional[str]):
        ...
        if thread_id is None:
            ...
            raise HelperError(
                ...
                "the iOS debugger did not expose any threads",
            )
        ...
        raise HelperError(
            "automation_unavailable",
            f"unknown iOS debugger thread id: {thread_id}",
        )
    ...
    def require_stopped_process(self):
        ...
        if self.state_value() != "stopped":
            raise HelperError(
                "automation_unavailable",
                "the iOS debugger process must be stopped before inspection",
            )
        return process
    ...
    def ensure_attached(self) -> None:
        if not self.attached:
            raise HelperError(
                "automation_unavailable",
                "the iOS debugger session is not attached",
            )
    ...
    def wait_for_state(self, expected: str, timeout_seconds: float) -> None:
        ...
        raise HelperError(
            "automation_unavailable",
            f"timed out waiting for the iOS debugger to become {expected}",
        )
    ...
    def process(self):
        if self.debugger is None:
            raise HelperError(
                "automation_unavailable",
                "the iOS debugger is not initialized",
            )
        target = self.debugger.GetSelectedTarget()
        if not target.IsValid():
            raise HelperError(
                "external_tool_failed",
                "LLDB did not select an iOS target",
            )
        process = target.GetProcess()
        if not process.IsValid():
            raise HelperError(
                "external_tool_failed",
                "LLDB did not expose an iOS process",
            )
        return process
    ...
    def lookup_app_process(self) -> Optional[int]:
        ...
        if result.returncode != 0:
            message = (
                ...
                or "failed to inspect simulator processes"
            )
