...
/// # Panics
///
/// Panics if the runtime singleton has not been initialized.
#[must_use]
...
mod tests {
    ...
    use super::{
        ...
        dispatch_event, ensure_running, lock_test_guard, set_state, state_value,
    };
    ...
    fn ensure_running_ok_when_running() {
        run_isolated(|| {
            __init(RuntimeConfig::default()).expect("init");
    ...
    fn reset_allows_reinitialization() {
        run_isolated(|| {
            ...
            __shutdown();
            __reset();
            __init(RuntimeConfig::default()).expect("reinit after reset");
        });
    }
