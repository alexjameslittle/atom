...
/// # Errors
///
/// Returns an error if the module source tree cannot be read, parsed, or mapped
/// to the supported `FlatBuffers` schema subset.
...
/// # Errors
///
/// Returns an error if the parsed Rust items cannot be rendered to a valid
/// `FlatBuffers` schema string.
    ...
    for record in &parsed.records {
        match record {
            Record::Struct(record) => {
                blocks.push(render_table(&record.name, &record.fields)?);
            }
            Record::SimpleEnum(record) => {
                blocks.push(render_simple_enum(record));
            }
            Record::DataEnum(record) => {
                ...
                blocks.push(render_union(record));
                blocks.push(render_table(
                    &record.name,
                    &[Field {
                        name: "value".to_owned(),
...
fn render_simple_enum(record: &SimpleEnumRecord) -> String {
    let mut output = format!("enum {}: ubyte {{\n", record.name);
    for variant in &record.variants {
        output.push_str("  ");
...
fn render_union(record: &DataEnumRecord) -> String {
    let mut output = format!("union {}Union {{\n", record.name);
    for variant in &record.variants {
        output.push_str("  ");
...
fn render_table(name: &str, fields: &[Field]) -> AtomResult<String> {
    let mut output = format!("table {name} {{\n");
    for field in fields {
        output.push_str("  ");
        output.push_str(&field.name);
        output.push_str(": ");
        output.push_str(&render_type(&field.ty)?);
        output.push_str(";\n");
    }
    output.push('}');
    Ok(output)
